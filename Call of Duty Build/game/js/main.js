...
import { vmAssets } from './vmassets.js';
import { upgradeTextures } from './materials.js';
...
async function boot() {
    const steps = [
        ...
        ['Deploying operators', loadCharacters],
...
function loop(ts) {
    ...
    if (fpsAccum >= 0.5) {
        ...
        if (strip && !strip.classList.contains('hidden')) {
            ...
            strip.innerHTML = `<b class="${cls}">${fps} FPS</b> · ${info.calls} draws · ` +
                `${(info.triangles / 1000).toFixed(0)}k tris · ${(curDpr * 100).toFixed(0)}% res · ` +
